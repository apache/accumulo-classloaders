 ...
 * {@link CargoManifest} files and the {@link Resource} objects it references.
 *
 * <p>
 ...
 * <ul>
 * <li><b>manifests</b>: stores a copy of the {@link CargoManifest} JSON files for each cargo
 * context, and exist primarily for user convenience (they aren't used again by this factory)
 * <li><b>resources</b>: stores a copy of all the {@link Resource} files for all cargo contexts
 ...
 * cargo contexts reference the file.
 *
 * <p>
 ...
 * using the same resource ordering as in the {@link CargoManifest} file.
 */
final class LocalStore {
  ...
  static String checksumForFileName(CargoManifest manifest) {
  ...
  /**
   * Save the {@link CargoManifest} to the manifests directory, and all of its resources to the
   * resources directory.
   */
  void storeCargo(final CargoManifest manifest) {
  ...
  private void storeManifest(final CargoManifest manifest, final String destinationName)
      throws IOException {
  ...
  Path createWorkingHardLinks(final CargoManifest manifest, Consumer<Path> forEachLink)
      throws HardLinkFailedException {
    Path hardLinkDir = createTempDirectory("cargo-" + checksumForFileName(manifest));
