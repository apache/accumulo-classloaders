...
return, this factory transports the cargo from the remote locations specified
...
Users may take advantage of the `CargoManifest.create(int,String,URL[])` method
to construct a `CargoManifest` object, programmatically. This will calculate
the checksums of the classpath elements. `CargoManifest.toJson()` can be used
to serialize the `CargoManifest` to a `String` to store in a file.

Alternatively, if this library's jar is built and placed onto Accumulo's
`CLASSPATH`, then one can run `bin/accumulo create-cargo-manifest` to create
the manifest using the command-line. The resulting json is printed to stdout
and can be redirected to a file. The command takes two arguments:
...
interval read from the most recent retrieval elapses. Changes to the cargo's
